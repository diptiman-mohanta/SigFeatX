...
_PadMode = Literal[
    'constant', 'edge', 'linear_ramp', 'maximum', 'mean',
...
class SignalUtils:
    ...
    def pad_signal(sig: np.ndarray, target_length: int,
                  mode: _PadMode = 'constant') -> np.ndarray:
        """
    ...
    @staticmethod
    def detect_peaks(sig: np.ndarray, height: float | None = None,
                    distance: int | None = None) -> tuple[np.ndarray, dict]:
        """
